...
| `user_preferences` | 20260830053143 | **外觀主題**（cream/noir，CHECK-constrained）— one row/user, `theme` + `updated_at`. 🪦 **2026-08-06 noir 主題整組下架後零讀寫端**——`src/stores/data-sync.ts` 的 `loadThemePreference`/`upsertThemePreference` 已刪除，表保留未 drop、CHECK constraint 不動（比照 `skill_scores`/`lesson_side_learned` 孤兒表先例，但無對應 drop migration，純粹閒置）。migration 本身仍未套用，套不套已無實質影響。 |
