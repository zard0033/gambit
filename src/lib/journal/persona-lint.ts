...
/** Blame / error vocabulary forbidden in solace entries (R8 永不批評). */
...
/**
 * Persona-lint a rendered body. Returns a list of violation codes (empty = clean).
 * - Always: no emoji; no xiangqi piece terms (車/馬/象).
 * - When `opts.solace`: additionally no blame/error tokens and no digits
 *   (solace must never criticise or quote eval numbers — R8 / AC-solace-3).
 *
...
/** Pen-aware convenience wrapper: applies the solace rules when `pen === 'solace'`. */
export function lintEntryBody(text: string, pen: Pen): string[] {
  return lintBody(text, { solace: pen === 'solace' })
}
