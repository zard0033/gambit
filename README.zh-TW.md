...
我用 Claude Code 寫的。324 個 commit 裡有 280 個帶著 `Co-Authored-By: Claude`，包含下面連出去的那幾個，你可以自己點開看。決定還是我下的：做什麼、砍什麼，以及後來哪一條我自己的主張該停止相信。最後那一項就是這份 README 接下來要講的，所以對程式碼怎麼來的避而不談，會很奇怪。
...
- **兩趟賽後分析**，逐手對照你走的手與最佳手
- **判斷場**：用你自己漏掉的強制勝局面出成盲測題，事先不說有沒有答案。誘餌局面（讓這題真的有兩種答案的那一半）目前只在手寫的概念題組裡，自家對局的生成器還做不出來
...
- 324 個 commit，橫跨 2026 年 2 月到 8 月
- 80 個測試檔（單元測試 ＋ E2E）
