...
| D3 難度五檔 | ~~砍成一檔~~ **已於 2026-08-06 撤銷**——原判死理由誤讀了實作：五檔的 `fallible.probability`／cp 帶本身是真旋鈕（rung 1 至 5 各自不同，非 skillLevel/depth 那三個啞彈），Eason 會隨棋力成長挑戰更高檔，選單保留，程式碼零改動 |
| D4 棋憶敘事外殼 | **刪表演層**（保留分析／missed-mate／MemoryDashboard） |
| D5 開局知識卡 ＋ opening-id | **已於 2026-08-06 縮小範圍執行**——只刪開局知識卡（4 檔，真死碼）；opening-id／chess-openings **保留**，原判死時（2026-08-02）「匯出這盤棋」還是死碼（D7 舊判決），D7 已於 2026-08-05 撤銷復活，`GameExportCard.vue` 現在靠 opening-id 產生 PGN `Opening`/`ECO` header 與 AI prompt 的開局句，砍了會讓每盤匯出少一行開局資訊，不是無痛清理 |
| D6 ProfileView | **已於 2026-08-06 執行**——整頁刪除；核實時發現 SoT 原案漏搬兩項真功能（`authStore.signOut()`／`/history` 連結都只有 ProfileView 一個入口，砍了會讓已登入使用者找不到登出），連同主題切換、重置對話框一起搬進新建的 header 齒輪 popover（`settings-menu.vue`，reka-ui Popover，四項：對局紀錄／外觀/重置對局記錄／登出），三個「即將推出」鎖住列不搬 |
...
**施工順序（依賴決定，不可任意調換）**：
零風險四項 → 判斷場搬遷（前置）→ D1 棋誌（須早於任何 game-history 動作）→ D2 → D4 → ~~D3~~（撤銷，見上表）→ D5 → D6。**全部項目已於 2026-08-06 執行或撤銷完畢**（D3 撤銷；D5 是縮小範圍執行，opening-id 保留，非全刪；細節見上表各條目）。
